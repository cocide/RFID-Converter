...
if ($_POST['method'] == 'hex') {
	...
	$bin = base_convert(substr($_POST['rfid'], -8), 10, 2).'0000';
	$known['bits'] = strlen(base_convert(substr($_POST['rfid'], 0, 1).str_repeat('9', (strlen(preg_replace("/[^0-9]/", "", $_POST['rfid'])) - 1) ), 10, 2).'0000') - 1;
	$known['padding'] = 4;
	...
	$bin = str_pad(base_convert($parts[0], 10, 2), 8, '0', STR_PAD_LEFT).str_pad(base_convert($parts[1], 10, 2), 16, '0', STR_PAD_LEFT).'0000';
	$known['bits'] = strlen($bin);
	$known['padding'] = 4;
...
$hex = base_convert($bin, 2, 16); //convert it to hex too

$section = substr(str_pad($bin, 30, '0', STR_PAD_LEFT), -28); // we only care about the last 28 bits
$id = base_convert(substr($section, 0, -4), 2, 10);

$facility = base_convert(substr($section, 0, 8), 2, 10);//the first 8 bits of the last 28 are the facility
$pin = base_convert(substr($section, 8, 16), 2, 10);//bits 9-24 of the last 28 are the pin
